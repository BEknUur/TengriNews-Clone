...
class NewsConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self) -> None:
        """Accept the websocket connection and subscribe the client to updates."""
    ...
    async def disconnect(self, code: int) -> None:
        """Remove the websocket client from update groups on disconnect."""
        await self.channel_layer.group_discard(NEWS_GROUP_NAME, self.channel_name)

    async def receive(self, text_data: str | None = None, bytes_data: bytes | None = None) -> None:
        """Handle an incoming websocket message payload from the client."""
